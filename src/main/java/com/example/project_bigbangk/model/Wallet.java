...
public class Wallet implements Cloneable {
    ...
    public double freeBalance(){
        ...
        for (Limit_Buy limit_buy : limitBuy) {
            reservedBalance += limit_buy.getOrderLimit() * limit_buy.getAssetAmount();
            reservedBalance += (limit_buy.getOrderLimit() * limit_buy.getAssetAmount() * BigBangkApplicatie.bigBangk.getFeePercentage());
        }
